...
- **`DEPLOY_NOOP` repo variable** (Settings → Secrets and variables → Actions → Variables) — until the SSH deploy secrets above are set, set this to `true` so `deploy-staging`/`deploy-production` skip the actual SSH step (which would otherwise fail CI with no real host to connect to) and just log that they were skipped. Remove it, or set it to anything other than `true`, once a real host is configured — this repo itself has it set to `true` for exactly that reason.
